...
- ## 作者要说
  - MCP模块类似于MCPserver，但是是直接注入解析后的变量和创造返回的方法，以前选则plugin是他看上去实现起来不难，但实则一点也不好实现，且自由度不高，选择模块后，AgentCLI会直接使用exec或evel函数直接在其他线程运行模块，并注入变量和函数，大致最终实现的功能会和`MCP模块说明`里说的差不多。
## Beta-1.1.2.2
- ## 更新
